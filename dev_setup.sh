...
docker-compose run web sh -c "rm -rf bower_components && cd /tmp && python /code/manage.py bower install && python /code/manage.py collectstatic --noinput && mv bower_components /code/"
